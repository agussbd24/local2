...
const statusOptions = [
  ...
  { value: 'preparing', label: 'Preparando', color: 'hover:border-amber-400/40 hover:bg-amber-500/10 hover:text-amber-200' },
  { value: 'ready', label: 'Listo', color: 'hover:border-emerald-400/40 hover:bg-emerald-500/10 hover:text-emerald-200' },
...
export default function OrderCard({ order, onStatusChange }) {
  ...
  return (
    ...
    >
      ...
      {order.notes && (
        <div className="mt-5 rounded-2xl border border-amber-300/15 bg-amber-300/5 p-4 text-sm">
          <div className="mb-2 flex items-center gap-2 font-bold text-amber-200">
            <StickyNote size={16} />
            Observaciones
          </div>
          <p className="leading-6 text-amber-50/80">{order.notes}</p>
        </div>
      )}
      ...
      <div className="mt-5 grid grid-cols-2 gap-2">
        {statusOptions.map((status) => (
          <button
            ...
            className={`rounded-xl border border-white/10 px-3 py-2.5 text-sm font-bold text-neutral-300 transition-all duration-300 disabled:cursor-default disabled:border-amber-300/30 disabled:bg-gradient-to-r disabled:from-amber-400 disabled:to-amber-300 disabled:text-neutral-950 ${status.color}`}
          >
            {status.label}
          </button>
        ))}
