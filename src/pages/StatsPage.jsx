                  ...
                  {stats.ordersByHour.map((h) => (
                    <div key={h.hour} className="flex flex-col items-center flex-1 gap-1">
                      <div className="w-full flex justify-center">
                        <div
                          className="w-full max-w-[24px] rounded-t-lg bg-gradient-to-t from-violet-500/60 to-violet-400/30 transition-all duration-500"
                ...
                <div className="flex items-end gap-2 h-48">
                  {stats.revenueByDay.map((d) => (
                    <div key={d.date} className="flex flex-col items-center flex-1 gap-1">
                      <div className="w-full flex justify-center">
                        <div
                          className="w-full max-w-[32px] rounded-t-lg bg-gradient-to-t from-emerald-500/60 to-emerald-400/30 transition-all duration-500"
                          style={{
                  ...
                  <div className="space-y-4">
                    {stats.salesByCategory.map((cat, index) => (
                      <div key={cat.category} className="flex items-center gap-4">
                        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-violet-400/20 to-violet-300/5 text-sm font-bold text-violet-200">
                          {index + 1}
                        </span>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center justify-between">
                            <p className="truncate font-bold text-white">{cat.label}</p>
                            <span className="ml-4 shrink-0 text-sm font-bold text-violet-200">
                              {formatCurrency(cat.revenue)}
                            </span>
                          </div>
                          <div className="mt-2 h-2 overflow-hidden rounded-full bg-white/5">
                            <div
                              className="h-full rounded-full bg-gradient-to-r from-violet-400 to-violet-300 transition-all duration-500"
                              style={{
                ...
                {stats.ordersByTable.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {stats.ordersByTable.map((t) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <span className="grid h-9 w-9 place-items-center rounded-lg bg-amber-300/10 text-sm font-bold text-amber-200">
                            {t.table}
                          </span>
